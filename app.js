...
const app = {
    ...
    showMyPosts: function() {
        ...
        this.callApi(data).then(response => {
            if (response.status === 'success' && response.items) {
                this.renderMyPosts(response.items);
            } else {
                container.innerHTML = `
                    <div class="text-center text-red-500 mt-10">
                        Error loading your posts: ${response.message || 'Unknown error'}
                    </div>
                `;
            }
        }).catch(error => {
    ...
    renderMyPosts: function(items) {
        ...
        if (!items || items.length === 0) {
            container.innerHTML = `
                <div class="text-center text-gray-500 mt-10">
                    ...
                    <button onclick="app.showView('post-item')" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                        Post an Item
                    </button>
                </div>
            `;
        ...
        let html = '';
        items.forEach(item => {
            const formattedDate = new Date(item.posted_at).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'short',
                day: 'numeric',
                hour: '2-digit',
            ...
            const statusClass = item.is_claimed === '1' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800';
            const statusText = item.is_claimed === '1' ? 'Claimed' : item.status || 'Active';
                    ...
                    <div class="p-4">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 mt-1">${item.type} • ${formattedDate}</p>
                                <span class="inline-block mt-2 px-2 py-1 rounded-full text-xs font-medium ${statusClass}">
                                    ${statusText}
                                </span>
                            </div>
                            ${item.image_url ? `
                                <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 ml-4">
                                    <img src="${item.image_url}" alt="${item.item_name}" class="w-full h-full object-cover">
                                </div>
                            ` : ''}
                        </div>
                        
                        ${item.description ? `<p class="mt-2 text-gray-600 text-sm">${item.description}</p>` : ''}
                        
                        <div class="mt-3 pt-3 border-t border-gray-100 flex justify-end space-x-2">
                            ${item.is_claimed !== '1' ? `
                                <button onclick="app.markAsClaimed('${item.item_id}')" class="px-3 py-1.5 bg-green-100 text-green-700 text-sm rounded-lg hover:bg-green-200 transition-colors">
                                    Mark as Claimed
                                </button>
                            ` : ''}
                            <button onclick="app.editItem('${item.item_id}')" class="px-3 py-1.5 bg-blue-100 text-blue-700 text-sm rounded-lg hover:bg-blue-200 transition-colors">
                                Edit
                            </button>
                            <button onclick="app.deleteItem('${item.item_id}')" class="px-3 py-1.5 bg-red-100 text-red-700 text-sm rounded-lg hover:bg-red-200 transition-colors">
                                Delete
                            </button>
                        </div>
                    </div>
    ...
    markAsClaimed: async function(itemId) {
    ...
    deleteItem: async function(itemId) {
        ...
        try {
            ...
            if (response.status === 'success') {
                ...
                this.showMessage(response.message || 'Failed to delete item');
            }
        } catch (error) {
            console.error('Error deleting item:', error);
            this.showMessage('An error occurred while deleting the item');
        }
    },
    ...
    claimItem: async function(itemId) {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                // Update in items array if it exists
                if (this.state.items) {
                ...
                if (window.location.hash === '#item-details') {
                    this.showDetails(itemId);
                }
                
                // Refresh all relevant views
                this.fetchItems('all');
                this.fetchItems('Found');
                if (this.state.currentView === 'my-posts') {
                    this.showMyPosts();
                }
                
            } else {
                throw new Error(result.message || 'Failed to mark item as claimed');
            }
        } catch (error) {
            console.error('Error claiming item:', error);
            this.showMessage(error.message || 'An error occurred while updating the item status');
            if (claimBtn) {
    ...
    renderConversations: function(conversations, container) {
        ...
        conversations.forEach(chat => {
            ...
            const chatHtml = `
                <div class="w-full text-left bg-white p-4 rounded-xl shadow-md mb-2 hover:bg-gray-50" 
    ...
    renderMessages: function(messages, container) {
        ...
        messages.forEach(msg => {
            ...
            let displayTime = '...';
            try {
                const phTime = new Date(msg.sent_at);
                displayTime = phTime.toLocaleTimeString('en-PH', { 
                    hour: '2-digit', 
                    minute: '2-digit',
                    hour12: true,
    ...
    markMessagesAsRead: function() {
        ...
        const data = {
            ...
            user_id: this.state.user.user_id, 
            other_user_id: this.state.currentChatReceiverId 
        };
...
function updatePhilippineTime() {
    try {
        ...
        if (timeElement) {
            const now = new Date();
            // Format time in PH timezone (Asia/Manila) To display at the top Nav
            const options = {
                timeZone: 'Asia/Manila',
                hour: '2-digit',
                minute: '2-digit',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleKeyPress(key) {
        ...
        if (key === 'backspace') {
            ...
            isShift = !isShift;
            const shiftBtns = document.querySelectorAll('[data-key]');
            shiftBtns.forEach(btn => {
                if (btn.dataset.key.length === 1) {
                    btn.textContent = isShift ? btn.dataset.key.toUpperCase() : btn.dataset.key.toLowerCase();
                }
            });
            ...
            if (isShift) {
                ...
                shiftBtns.forEach(btn => {
                    if (btn.dataset.key.length === 1) {
                        btn.textContent = btn.dataset.key.toLowerCase();
                    }
                });
            }
        }
    ...
    if (keyboard && input && toggleKeyboardBtn) {
    ...
    setTimeout(updatePhilippineTime, 500);
});