...
switch ($action) {
    ...
    case 'claim_item':
        ...
        try {
            ...
            // 2. UPDATE ITEM STATUS - Set as claimed and update type to 'Found'
            $sql = "UPDATE items 
                    SET is_claimed = '1', 
                        status = 'Recovered',
                        type = 'Found',
...
case 'mark_messages_read':
    if (empty($data->item_id) || empty($data->user_id) || empty($data->other_user_id)) {
        http_response_code(400);
    ...
    try {
        ...
        $result = $stmt->execute([
            ':item_id' => $data->item_id,
            ':sender_id' => $data->other_user_id, // The person who sent the messages
            ':receiver_id' => $data->user_id     // The current user who is reading them
        ]);

        if ($result) {
            $count = $stmt->rowCount();
            echo json_encode(['status' => 'success', 'message' => "$count messages marked as read"]);
        } else {
    ...
    try {
        ...
        $sql = "
            SELECT 
                ...
                m.sent_at AS last_message_time_raw,
